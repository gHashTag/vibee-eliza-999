...
# Change ownership only on source and build files (excluding node_modules for performance)
# node_modules ownership is not critical - files are already readable
RUN chown -R node:node /app/dist /app/src /app/package.json /app/bun.lock* /app/tsconfig*.json /app/vite.config.ts /app/build.ts 2>/dev/null || true
