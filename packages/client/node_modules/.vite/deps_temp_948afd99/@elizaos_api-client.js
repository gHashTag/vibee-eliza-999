...
var BaseApiClient = class {
  ...
  async request(method, path, options) {
    ...
    try {
      const headers = {
        ...this.defaultHeaders,
        ...(_a = options == null ? void 0 : options.config) == null ? void 0 : _a.headers,
        ...options == null ? void 0 : options.headers
      };
      if ((options == null ? void 0 : options.body) instanceof FormData) {
        delete headers["Content-Type"];
      }
      const response = await fetch(url.toString(), {
        method,
        headers,
        body: (options == null ? void 0 : options.body) instanceof FormData ? options.body : (options == null ? void 0 : options.body) ? JSON.stringify(options.body) : void 0,
      ...
      try {
        ...
        if (response.ok) {
          ...
          throw new ApiError("PARSE_ERROR", "Failed to parse response as JSON", void 0, response.status);
        }
      }
      if (!response.ok) {
        const error = (jsonData == null ? void 0 : jsonData.error) || {
          code: "HTTP_ERROR",
...
var MessagingService = class extends BaseApiClient {
  ...
  async addUserToChannel(channelId, userId) {
    ...
    const currentParticipants = ((_a = channel.metadata) == null ? void 0 : _a.participantCentralUserIds) || [];
...
var SessionsService = class extends BaseApiClient {
  ...
  async sendMessage(sessionId, params) {
    validateRequiredParam(sessionId, "sessionId");
    validateRequiredParam(params == null ? void 0 : params.content, "content");
  ...
  async getMessages(sessionId, params) {
    ...
    const beforeTimestamp = toTimestampString(params == null ? void 0 : params.before, "before");
    if (beforeTimestamp) {
      queryParams.before = beforeTimestamp;
    }
    const afterTimestamp = toTimestampString(params == null ? void 0 : params.after, "after");
    if (afterTimestamp) {
...
var RunsService = class extends BaseApiClient {
...
var JobsService = class extends BaseApiClient {
  async create(params) {
  ...
  async poll(jobId, options = {}) {
    ...
    while (attempts < effectiveMaxAttempts) {
      ...
      if (job2.status === "failed" || job2.status === "timeout") {
        return {
          success: false,
          job: job2,
          attempts,
...
//# sourceMappingURL=@elizaos_api-client.js.map
