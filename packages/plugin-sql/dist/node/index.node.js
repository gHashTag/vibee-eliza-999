...
class BaseDrizzleAdapter extends DatabaseAdapter {
  ...
  async runPluginMigrations(plugins, options) {
    ...
    if (!skipMigrations) {
      ...
      logger8.info("[SQL Plugin] Skipping migrations (SKIP_SQL_MIGRATIONS=true)");
    }
  }
  getDatabase() {
    return this.db;
  ...
  async createMessage(data) {
    return this.withDatabase(async () => {
      ...
      logger8.debug(`[DB] createMessage - messageToInsert before insert: ${JSON.stringify({
      ...
      logger8.debug(`[DB] createMessage - messageToInsert after insert: ${JSON.stringify({
        id: messageToInsert.id,
        channelId: messageToInsert.channelId,
        authorId: messageToInsert.authorId,
        hasChannelId: "channelId" in messageToInsert,
...
//# debugId=2D14E96939EB126164756E2164756E21
