import { logger, parseBooleanFromText, getDatabaseDir, getGeneratedDir, getUploadsAgentsDir, ElizaOS, loadEnvFile, } from '@elizaos/core';
...
export function resolvePgliteDir(dir, fallbackDir) {
...
const DEFAULT_SERVER_ID = '00000000-0000-0000-0000-000000000000'; // Single default server
/**
 ...
 */
export function isWebUIEnabled() {
...
export class AgentServer {
    ...
    isWebUIEnabled = true; // Default to enabled until initialized
    clientPath; // Optional path to client dist files
    elizaOS; // Core ElizaOS instance (public for direct access)
    database;
    rlsOwnerId;
            ...
            */
            ...
            if (this.isWebUIEnabled) {
                ...
                const possiblePaths = [
                    ...
                    (() => {
                        try {
                            return path.resolve(path.dirname(require.resolve('@elizaos/client/package.json')), 'dist');
                        }
                        catch {
                            return null;
                        }
                    ...
                    (() => {
                        try {
                            ...
                            const bunGlobalPath = path.join(os.homedir(), '.bun/install/global/node_modules/@elizaos/server/dist/client');
                            ...
                            try {
                                ...
                                if (existsSync(nvmPath)) {
                                    ...
                                    for (const version of versions) {
                                        const cliPath = path.join(nvmPath, version, 'lib/node_modules/@elizaos/server/dist/client');
                ...
                if (clientPath) {
                    ...
                    possiblePaths.forEach((p) => {
                        if (p)
                            logger.warn(`[STATIC]   - ${p}`);
                    });
                    logger.warn('[STATIC] The web UI will not be available.');
                    logger.warn('[STATIC] To fix this, ensure the client is built: cd packages/client && bun run build');
        ...
        try {
            ...
            try {
                if (messageBusConnectorPlugin) {
                    ...
                    logger.info(`[AgentServer] Registered MessageBusConnector for agent ${runtime.character.name}`);
                }
                else {
                ...
                logger.error({ error: e }, `[AgentServer] CRITICAL: Failed to register MessageBusConnector for agent ${runtime.character.name}`);
            }
            ...
            try {
                ...
                logger.error({ error: e }, `[AgentServer] WARNING: Failed to register TEXT_LARGE model for agent ${runtime.character.name}`);
            }
            ...
            logger.success(`Successfully registered agent ${runtime.character.name} (${runtime.agentId}) with core services.`);
            await this.addAgentToServer(this.serverId, runtime.agentId);
            logger.info(`[AgentServer] Auto-associated agent ${runtime.character.name} with server ID: ${this.serverId}`);
        }
        catch (error) {
        ...
        try {
            ...
            if (agent) {
                ...
                try {
                    ...
                    logger.error({ error: stopError, agentId }, `[AGENT UNREGISTER] Error stopping agent services for ${agentId}:`);
                }
            }
    ...
    async resolveAndFindPort(port) {
        ...
        if (envPort) {
            ...
            if (!isNaN(parsed) && parsed >= 1 && parsed <= 65535) {
                ...
                logger.warn(`Invalid SERVER_PORT "${envPort}", using platform/default port ${requestedPort}`);
            }
        }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.server
                    ...
                    logger.success(`REST API bound to ${host}:${port}. If running locally, access it at http://localhost:${port}.`);
                    const agents = this.elizaOS?.getAgents() || [];
    ...
    async removeParticipantFromChannel() {
        ...
        logger.warn(`[AgentServer] Remove participant operation not directly supported in database adapter`);
    }
    // ===============================
