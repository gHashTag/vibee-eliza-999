...
export class InfisicalSecretLoader {
     ...
     */
    async loadSecrets(config, forceReload) {
     ...
     */
    async _loadSecretsInternal(config) {
     ...
     */
    async _fetchSecretsFromInfisical(config) {
        const secrets = new Map();
        try {
            // Get access token
 ...
 */
export async function loadInfisicalSecrets(config, forceReload) {
