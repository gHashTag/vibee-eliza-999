    ...
    checkEnvironment() {
        ...
        if (validation.missingCritical.length === 0 && validation.errors.length === 0) {
            ...
            return {
                ...
                details: {
                    loaded: validation.missingCritical.length === 0
            ...
            if (error.code === 'ECONNREFUSED') {
                ...
                if (postgresUrl) {
                    ...
                    try {
                        ...
                        console.log(`      - Заканчивается на: "...${postgresUrl.substring(postgresUrl.length - 30)}"`);
                ...
                console.log(`      - Неправильный POSTGRES_URL в Infisical Cloud (environment: ${process.env.INFISICAL_ENVIRONMENT || 'dev'})`);
                console.log(`      - База данных не запущена или недоступна`);
                console.log(`      - Проблемы с сетью или файрволом`);
                console.log(`      - Проверьте, что POSTGRES_URL в Infisical Cloud указывает на доступную БД`);
            }
            return {
                status: 'error',
