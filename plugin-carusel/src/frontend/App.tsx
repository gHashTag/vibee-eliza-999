...
const App: React.FC = () => {
  ...
  const handleCreateCarousel = React.useCallback(async (story: string) => {
    ...
    try {
        ...
        console.log("[DEBUG] handleCreateCarousel - starting PARALLEL image generation");
        ...
        setLoadingMessage(`${TRANSLATIONS.RENDERING[language]} [0/${newSlides.length}]...`);

        const imagePromises = newSlides.map(async (slide, i) => {
            console.log(`[DEBUG] Starting generation for slide ${i}: ${slide.imagePrompt?.substring(0, 50)}...`);
            try {
                const imageUrl = await generateImage(slide.imagePrompt, settings.artStyle, referenceImage || undefined, referenceType);
                console.log(`[DEBUG] Slide ${i} completed successfully`);
                return { index: i, imageUrl, success: true };
            } catch (e) {
                console.error(`[DEBUG] Failed to generate image for slide ${i}:`, e);
                return { index: i, imageUrl: null, success: false, error: e };
            }
        });

        // Use Promise.allSettled to wait for all and handle failures gracefully
        const results = await Promise.allSettled(imagePromises);

        let successCount = 0;
        results.forEach((settledResult, i) => {
            if (settledResult.status === 'fulfilled') {
                const result = settledResult.value;
                if (result.success && result.imageUrl) {
                    updatedSlides[result.index] = { ...updatedSlides[result.index], imageUrl: result.imageUrl };
                    successCount++;
                }
            }
        ...
        console.log(`[DEBUG] Generation complete: ${successCount}/${newSlides.length} successful`);
  ...
  const handleGenerateSingleImage = React.useCallback(async (slideId: string) => {
      ...
      try {
        console.log("[DEBUG] handleGenerateSingleImage called");
        console.log("[DEBUG] referenceImage state:", referenceImage ? `[BASE64 ${referenceImage.length} chars]` : "null");
        console.log("[DEBUG] referenceType:", referenceType);
  ...
  // Helper function to ensure fonts are loaded before export
  const exportToBlob = async (node: HTMLElement, options: any): Promise<Blob | null> => {
  ...
  const exportToPng = async (node: HTMLElement, options: any): Promise<string> => {
    // Wait for all fonts to load
    await document.fonts.ready;
    // Small delay to ensure fonts are rendered
    await new Promise(r => setTimeout(r, 150));
  ...
  const handleDownloadCarousel = React.useCallback(async () => {
    ...
    try {
      for (let i = 0; i < slides.length; i++) {
        ...
        if (node) {
          const blob = await exportToBlob(node, {
  ...
  const handleDownloadPDF = React.useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        const node = document.getElementById(`download-container-${slide.id}`);
        if (node) {
          const dataUrl = await exportToPng(node, {
            quality: 1.0, pixelRatio: 2,
            width: downloadDimensions.width, height: downloadDimensions.height,
            backgroundColor: settings.overlayMode === 'solid' ? settings.overlayColor : undefined,
            style: { transform: 'none' }
