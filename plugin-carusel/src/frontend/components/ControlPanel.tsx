              ...
              <div className="space-y-6 mb-8">
                  <div className="border border-white/10 p-4 rounded-lg bg-white/5">
                       ...
                       <input ref={refImageInputRef} type="file" hidden accept="image/*" onChange={(e) => {
                           ...
                           if(f) {
                             console.log("[DEBUG] Reference file selected:", f.name, f.type, f.size, "bytes");
                             const r = new FileReader();
                             r.onload=(ev)=> {
                               const result = ev.target?.result as string;
                               console.log("[DEBUG] Reference image loaded, length:", result?.length, "chars");
                               console.log("[DEBUG] Reference image prefix:", result?.substring(0, 50));
