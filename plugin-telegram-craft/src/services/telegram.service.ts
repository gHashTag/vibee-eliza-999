...
// Импорт сервиса пересылки диалогов
import { DialogForwardService, dialogForwardService, type DialogMetadata } from "./dialogForward.service";
import { findTriggersByCategory, containsTriggerCategory, findTriggers, getTriggerCategory, TRIGGER_CATEGORIES, containsTrigger } from "../config/triggers";

// Импорт generateImageAction напрямую из plugin-vibe-face-avatar
// (т.к. ElizaOS не регистрирует actions из этого плагина в runtime.actions)
import { generateImageAction } from "plugin-vibe-face-avatar";

// 🔒 ГЛОБАЛЬНЫЙ КЭШ ДЕДУПЛИКАЦИИ: Единый Set на весь процесс
// Это решает проблему параллельных вызовов из адаптера
const globalProcessedMessageIds = new Set<string>();
const GLOBAL_DEDUP_TTL = 60000; // 60 секунд - время жизни кэша
...
// 🔒 ГЛОБАЛЬНЫЙ SINGLETON: Один TelegramService на весь процесс
let globalTelegramServiceInstance: TelegramService | null = null;
...
export class TelegramService extends Service {
   ...
   * 🔒 SINGLETON: Возвращает существующий instance если уже создан
   */
  static async start(runtime: IAgentRuntime): Promise<Service> {
    console.log("🚀 [TelegramService] STATIC start() called");

    // 🔒 SINGLETON CHECK: Если уже есть instance - возвращаем его
    if (globalTelegramServiceInstance) {
      console.log("🔒 [TelegramService] Returning existing singleton instance (skipping duplicate initialization)");
      return globalTelegramServiceInstance;
    }

    console.log("🆕 [TelegramService] Creating new singleton instance");
    const instance = new TelegramService();
    globalTelegramServiceInstance = instance;
  ...
  protected runtime: IAgentRuntime | null = null; // 🔥 Для генерации ответов
  ...
  private dialogForwardService = dialogForwardService;
  
  // Anti-Loop State
  private repetitionMap = new Map<string, { count: number, lastText: string, lastTime: number }>();


  capabilityDescription =
    "Telegram userbot управление через MTProto (GramJS) с fallback стратегиями";
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    console.log(
      "🔑 [TelegramService] Runtime received: " + (runtime ? "YES" : "NO")
    );
    ...
    this.strategy =
      (runtime.getSetting("TELEGRAM_STRATEGY") as any) || "mtproto";
    ...
    try {
      ...
      console.log(
        `✅ [TelegramService] Service initialized successfully with ${this.strategy}`
      );
      console.log(
        `✅ [TelegramService] Adapter is ${this.adapter ? "SET" : "NULL"}`
      );
      ...
      KolsLogger.success(
        `Knowledge Base загружен: ${kbStats.sections} секций, ${kbStats.chunks} чанков`
      );
  ...
  async start(): Promise<void> {
    ...
    if (!this.adapter) {
      console.log("🔄 [TelegramService] Adapter not initialized, trying from ENV...");
      try {
        await this.initializeFromEnv();
      } catch (error) {
        console.warn(`⚠️ [TelegramService] Failed to initialize from ENV: ${error}`);
    ...
    try {
      ...
      if (connected) {
        console.log("✅ [TelegramService] Successfully connected to Telegram");
        
        // Регистрируем обработчик сообщений
        this.adapter.onMessage(async (message: ITelegramMessage) => {
          await this.processMessage(message);
        });
        
        console.log("📡 [TelegramService] Message handler registered");
      } else {
        console.warn("⚠️ [TelegramService] Failed to connect to Telegram");
      }
    } catch (error) {
      console.error(`❌ [TelegramService] Connection failed: ${error}`);
    }
  }
  private async initializeMTProto(runtime: IAgentRuntime): Promise<void> {
    ...
    const session =
      runtime.getSetting("TELEGRAM_SESSION_STRING") ||
    ...
    if (!apiId || !apiHash) {
      throw new Error(
        "TELEGRAM_API_ID и TELEGRAM_API_HASH обязательны для MTProto"
      );
    }

    console.log("🔧 [MTProto] Getting adapter singleton instance...");
    this.adapter = MTProtoAdapter.getInstance({
      apiId: parseInt(apiId),
      apiHash,
      session,
    });
    console.log("✅ [MTProto] Adapter singleton instance obtained");
  }
  ...
  private async initializeBotApi(runtime: IAgentRuntime): Promise<void> {
    ...
    this.adapter = new BotApiAdapter(botToken);
  }
  ...
  private async initializeMcp(runtime: IAgentRuntime): Promise<void> {
    this.adapter = new McpAdapter(""); // TODO: Add MCP server URL setting
  }
  ...
  async start(): Promise<void> {
    ...
    if (!this.adapter) {
      console.log(
        "⚠️ [TelegramService] Adapter not initialized, trying ENV..."
      );
    ...
    if (!this.adapter) {
      console.warn(
        "⚠️ [TelegramService] No adapter available - Telegram features disabled"
      );
      console.warn(
        "⚠️ [TelegramService] Add TELEGRAM_API_ID/HASH to ENV or character secrets"
      );
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error(
          "⚠️ [TelegramService] Failed to auto-start monitoring:",
          result.error
        );
      }
    } catch (error) {
      ...
      console.warn(
        "⚠️ [TelegramService] Continuing without Telegram connection"
      );
    }
  }
  ...
  private async initializeFromEnv(): Promise<void> {
    ...
    this.strategy = strategy;
      ...
      console.log(
        `🔧 [TelegramService] Initializing with strategy: ${this.strategy}`
      );

      // 🔥 FIX: Explicitly set TelegramService reference for DialogForwardService
      // This fixes "No way to send summary" error
      dialogForwardService.setTelegramService(this);
      console.log("✅ [TelegramService] DialogForwardService linked");
      ...
      console.error(
        `❌ [TelegramService] Failed to initialize from ENV:`,
        error
      );
      throw error;
    }
  ...
  private async initializeMTProtoFromEnv(): Promise<void> {
    ...
    console.log(
      `🔧 [MTProto] API ID from ENV: ${apiId ? "✅ SET" : "❌ MISSING"}`
    );
    console.log(
      `🔧 [MTProto] API Hash from ENV: ${apiHash ? "✅ SET" : "❌ MISSING"}`
    );
    console.log(
      `🔧 [MTProto] Session from ENV: ${session ? "✅ SET" : "⚠️ OPTIONAL"}`
    );
    ...
    if (!apiId || !apiHash) {
      ...
      console.warn(
        "⚠️ [MTProto] TELEGRAM_API_ID/HASH не найдены в ENV - MTProto отключен"
      );
      console.warn(
        "⚠️ [MTProto] Credentials будут загружены из character secrets при initialize()"
      );
      return; // Не throw, просто выходим
    }

    console.log("🔧 [MTProto] Getting adapter singleton instance from ENV...");
    this.adapter = MTProtoAdapter.getInstance({
      apiId: parseInt(apiId),
      apiHash,
      session,
    });
    console.log("✅ [MTProto] Adapter singleton instance obtained from ENV");
  }
  ...
  private async initializeMcpFromEnv(): Promise<void> {
    this.adapter = new McpAdapter(""); // TODO: Add MCP server URL from env
  }
  ...
  // Кэш для отслеживания ошибок прав доступа (чтобы не спамить логи)
  private forbiddenChatsCache = new Set<string>();
  private lastForbiddenLogTime = new Map<string, number>();
  private readonly FORBIDDEN_LOG_INTERVAL = 60000; // Логировать не чаще раза в минуту

  /**
   * Очистить кэш запрещённых чатов (например, после восстановления прав)
   */
  clearForbiddenChatsCache(chatId?: string): void {
    if (chatId) {
      this.forbiddenChatsCache.delete(chatId);
      this.lastForbiddenLogTime.delete(chatId);
      console.log(
        `✅ [TelegramService] Кэш запрещённых чатов очищен для ${chatId}`
      );
    } else {
      this.forbiddenChatsCache.clear();
      this.lastForbiddenLogTime.clear();
      console.log(
        `✅ [TelegramService] Кэш запрещённых чатов полностью очищен`
      );
    }
  }
  ...
  async sendMessage(
    ...
    if (allowedGroupId && chatId !== allowedGroupId) {
      console.log(
        `⛔ [TelegramService] БЛОКИРОВКА: Попытка написать в чужую группу: ${chatId} (разрешена только ${allowedGroupId})`
      );
      return {
        success: false,
    ...
    // Если чат уже в кэше запрещённых - пропускаем без попытки отправки
    if (this.forbiddenChatsCache.has(chatId)) {
      const lastLogTime = this.lastForbiddenLogTime.get(chatId) || 0;
      const now = Date.now();
      if (now - lastLogTime > this.FORBIDDEN_LOG_INTERVAL) {
        console.warn(
          `⚠️ [TelegramService] Пропуск отправки в чат ${chatId}: нет прав на запись (кэш)`
        );
        this.lastForbiddenLogTime.set(chatId, now);
      }
      return {
        success: false,
        error: `CHAT_WRITE_FORBIDDEN: Нет прав на отправку сообщений в чат ${chatId}`,
      };
    }

    const result = await this.adapter.sendMessage(chatId, message, replyTo);

    // Если получили ошибку прав доступа - добавляем в кэш
    if (
      !result.success &&
      (result.error?.includes("CHAT_WRITE_FORBIDDEN") ||
        result.error?.includes("403"))
    ) {
      this.forbiddenChatsCache.add(chatId);
      this.lastForbiddenLogTime.set(chatId, Date.now());
      console.warn(
  ...
  /**
   * Отправить сообщение с URL-кнопками (для оплаты и внешних ссылок)
   * @param chatId - ID чата
   * @param message - текст сообщения
   * @param buttons - массив рядов кнопок [[{text, url}]]
   */
  async sendMessageWithUrlButtons(
    ...
    if (allowedGroupId && chatId !== allowedGroupId) {
      console.log(
        `⛔ [TelegramService] БЛОКИРОВКА: Попытка написать в чужую группу: ${chatId}`
      );
      return {
        success: false,
        error: `Group ${chatId} is not allowed. Only ${allowedGroupId} is permitted.`,
    ...
    if (this.adapter instanceof MTProtoAdapter) {
      return this.adapter.sendMessageWithUrlButtons(chatId, message, buttons);
    }

    // Fallback: просто отправляем текст
    console.warn(
      "[TelegramService] sendMessageWithUrlButtons not supported by current adapter, sending text only"
    );
    return this.adapter.sendMessage(chatId, message);
  }
  ...
  async getMessages(
    ...
    console.warn(
      "[TelegramService] getMessages not supported by current adapter"
    );
    return [];
  }
  ...
  async getGroupMembers(
    ...
    console.warn(
      "[TelegramService] getGroupMembers not supported by current adapter"
    );
    return [];
  }
  ...
  async getUserAvatar(userId: string): Promise<string | null> {
    ...
    if (this.adapter instanceof MTProtoAdapter) {
      ...
      if (!buffer || buffer.length < 100) {
        console.log(
          `[TelegramService] No avatar for user ${userId} (buffer: ${buffer?.length || 0} bytes)`
        );
        return null;
      }
      ...
      console.log(
        `[TelegramService] Got avatar for user ${userId}: ${buffer.length} bytes`
      );
      return `data:image/jpeg;base64,${base64}`;
    }

    console.warn(
      "[TelegramService] getUserAvatar not supported by current adapter"
    );
    return null;
  }
    ...
    if (this.adapter instanceof MTProtoAdapter) {
      ...
      try {
        ...
        if (photoUrl.startsWith("data:")) {
          ...
          console.log(
            `[TelegramService] Converted data URL to Buffer (${file.length} bytes)`
          );
        } else if (
          ...
          console.log(
            `[TelegramService] Downloading image from: ${photoUrl.substring(0, 80)}...`
          );
          ...
          console.log(
            `[TelegramService] Downloaded image (${file.length} bytes)`
          );
        } else {
          throw new Error(
            `Unsupported photo URL format: ${photoUrl.substring(0, 30)}...`
          );
        }
        ...
        try {
          console.log(
            `[TelegramService] Attempting to send photo to chatId: ${chatIdNum} (as number)`
          );
          ...
          console.log(
            `✅ [TelegramService] Photo sent successfully to ${chatIdNum}, messageId: ${result.id}`
          );
          return { success: true, messageId: result.id };
        } catch (firstError: any) {
          console.log(
            `⚠️ [TelegramService] Failed to send to ${chatIdNum}, trying channel format. Error: ${firstError?.message || firstError}`
          );
          // Пробуем с -100 префиксом для channels
          try {
            const channelId = BigInt(`-100${chatId}`);
            console.log(
              `[TelegramService] Attempting to send photo to channel: ${channelId}`
            );
            ...
            console.log(
              `✅ [TelegramService] Photo sent successfully to channel ${channelId}, messageId: ${result.id}`
            );
            return { success: true, messageId: result.id };
          } catch (secondError: any) {
            console.error(
              `❌ [TelegramService] Both strategies failed. First error: ${firstError?.message || firstError}, Second error: ${secondError?.message || secondError}`
            );
            throw secondError; // Пробрасываем последнюю ошибку
          }
        }
      } catch (error) {
  ...
  // 🔒 ДЕДУПЛИКАЦИЯ: Используем глобальный globalProcessedMessageIds (определён в начале файла)
  // Это решает проблему дублирования при параллельных вызовах из адаптера
  ...
  addLiveFeedCallback(callback: (message: string) => void): void {
    ...
    console.log(
      `📺 [TelegramService] Live feed callback добавлен (всего: ${this.liveFeedCallbacks.size})`
    );
  }
  ...
  removeLiveFeedCallback(callback: (message: string) => void): void {
    ...
    console.log(
      `📺 [TelegramService] Live feed callback удален (осталось: ${this.liveFeedCallbacks.size})`
    );
  }
  ...
  private sendToLiveFeed(message: any): void {
    try {
      ...
      // 🎨 Форматированный вывод с цветами
                const header = `${colorize.time(`[${time}]`)} 📨 ${colorize.chat(message.tgChatTitle)}`;      const sender = `${colorize.user(message.fromFirstName)}${message.fromUsername ? colorize.meta(` ${message.fromUsername}`) : ""}`;
      const messageText = colorize.message(
        `${message.text.substring(0, 150)}${message.text.length > 150 ? "..." : ""}`
      );
      ...
      this.liveFeedCallbacks.forEach((callback) => {
        try {
          ...
          console.error(
            colorize.error("❌ [TelegramService] Live feed callback error:"),
            error
          );
        }
      });
      ...
      if (this.messageDistributor) {
        try {
          ...
          console.error(
            colorize.error("❌ [TelegramService] Message distributor error:"),
            error
          );
        }
      }
      ...
      console.error(
        colorize.error("❌ [TelegramService] Failed to send to live feed:"),
        error
      );
    }
  }
   ...
   */
  async startGroupMonitoring(): Promise<{
    success: boolean;
    message: string;
    error?: string;
  }> {
   ...
   */
  async removeGroupFromMonitoring(
    groupId: string
  ): Promise<{ success: boolean; message: string }> {
    ...
    if (removed) {
      ...
      return {
        success: true,
        message: `Группа ${groupId} удалена из мониторинга`,
      };
    } else {
  ...
  /**
        const staticConfig = AGENTS_CONFIG.vibee?.targetChats.find(c => c.chatId === finalChatId);
        if (staticConfig?.forwardChatId && chatConfig?.forwardChatId !== staticConfig.forwardChatId) {
             console.log(`🔄 [TelegramService] Force updating forwardChatId from static config: ${chatConfig?.forwardChatId} -> ${staticConfig.forwardChatId}`);
             if (chatConfig) {
                 chatConfig.forwardChatId = staticConfig.forwardChatId;
             }
        }

        console.log(
          `🎯 [TelegramService] ChatConfigService config for ${finalChatId}: ${chatConfig ? "FOUND" : "NOT FOUND"}`
        );
      }
      ...
      // 📸 ФОТО-СЕССИИ: Накапливаем фото для генерации (ТОЛЬКО ЕСЛИ РАЗРЕШЕНО)
      if (chatConfig?.allowImages !== false) {
        // Для личных чатов используем chatId как sessionUserId (в DM chatId = userId собеседника)
        // Для групп используем fromUserId
        const isPrivateChatCheck = isPrivateChat(finalChatId);
        const sessionUserId = isPrivateChatCheck
          ? finalChatId
          : fromUserId || finalChatId;

        const photoSessionHandled = await this.handlePhotoSession(
          message,
          finalChatId,
          sessionUserId
        );
      ...
      console.log(
        `🎯 [TelegramService] Chat ID check: raw=${message.chatId || "unknown"}, final=${finalChatId}, title=${tgChatTitle}`
      );
      console.log(
        `🎯 [TelegramService] isTargetFromConfig=${isTargetFromConfig}, isTargetFromStatic=${isTargetFromStatic}, final=${isTarget}`
      );

      // Дополнительная диагностика для отладки
      if (!isTarget) {
        console.log(
          `⏭️ [TelegramService] ПРОПУСК: Чат ${finalChatId} (${tgChatTitle}) не в целевом списке`
        );
        console.log(
          `🔍 [TelegramService] Диагностика: chatConfig=${chatConfig ? "exists" : "null"}, isActive=${chatConfig?.isActive ?? "N/A"}, shouldProcessChat(${finalChatId})=${isTargetFromStatic}`
        );
        console.log(
          `💡 [TelegramService] Чтобы добавить чат, используйте команду /config_add или добавьте в agents.config.ts`
        );
        return;
      }

      // Проверяем triggers и probability
      // 🕉️ КРИТИЧНО: Для личных чатов ВСЕГДА отвечаем, независимо от конфига
      const isPrivate = isPrivateChat(finalChatId);

      // Для всех чатов используем shouldRespondInChat из agents.config.ts
      if (!isPrivate) {
        const shouldRespond = shouldRespondInChat(finalChatId, "vibee", processedMessage.text || "", false);
        if (!shouldRespond) {
          console.log(
            `⏭️ [TelegramService] ПРОПУСК: Триггеры не сработали`
          );
          return;
        }
        console.log(`🎯 [TelegramService] Сработали триггеры из agents.config.ts`);
      }

      if (isPrivate) {
        console.log(
          `💬 [TelegramService] ЛИЧНЫЙ ЧАТ: ${finalChatId} - всегда отвечаем`
        );
      }
      ...
      KolsLogger.success(
        `ЦЕЛЕВОЙ ЧАТ: ${tgChatTitle} (${finalChatId})${chatConfig ? " [из БД]" : " [статический]"}`
      );

      console.log(
        `🔍 [TelegramService] Should monitor this chat: ${shouldMonitor}`
      );
      ...
      if (shouldMonitor) {
        ...
        console.log(
          `${colorize.time(`[${timestamp}]`)} 📨 ${colorize.chat(tgChatTitle)} » ${colorize.user(fromFirstName)}${usernameDisplay ? colorize.meta(` ${usernameDisplay}`) : ""}: ${colorize.message(processedMessage.text?.substring(0, 100) || "...")}`
        );

        // Проверяем триггерные слова
        this.checkTriggerWords(processedMessage);

        // 🛡️ КРИТИЧНО: Проверяем права на запись в чат ПЕРЕД обработкой
        if (!canWriteToChat(finalChatId)) {
          console.log(
            `🚫 [TelegramService] ПРОПУСК ОТВЕТА: Нет прав на запись в чат ${finalChatId} (${tgChatTitle}). Только мониторинг.`
          );
          return;
        }
        ...
        console.log(
          `🔥 [TelegramService] BEFORE handleMessageThroughActions - runtime: ${this.runtime ? "SET" : "NULL"}, message: "${processedMessage.text?.substring(0, 50)}"`
        );
        try {
          await this.handleMessageThroughActions(processedMessage, chatConfig);
          console.log(
            `✅ [TelegramService] AFTER handleMessageThroughActions - успешно`
          );
        } catch (actionError) {
          console.error(
            `❌ [TelegramService] Ошибка в handleMessageThroughActions:`,
            actionError
          );
  ...
  private checkTriggerWords(message: any): void {
    ...
    if (containsTrigger(text)) {
      ...
      KolsLogger.info(`Чат: ${message.tgChatTitle}`);
   ...
   */
  private shouldRespondWithConfig(
    config: ChatConfig,
    messageText: string
  ): boolean {
   ...
   */
  private async handleMessageThroughActions(
    ...
    try {
      ...
      const entityId = stringToUuid(
        `telegram-user-${processedMessage.fromUserId}`
      );
      ...
      const memory: Memory = {
        id: stringToUuid(
          `telegram-${processedMessage.chatId}-${processedMessage.messageId}-${Date.now()}`
        ),
      ...
      const callback = async (response: { text: string }) => {
        if (response.text) {
          KolsLogger.success(
            `🎯 Action ответ: "${response.text.substring(0, 50)}..."`
          );
      ...
      const runtimeActions = this.runtime.actions || [];

      // Добавляем generateImageAction если его нет в runtime.actions
      // (ElizaOS не регистрирует actions из plugin-vibe-face-avatar)
      const hasGenerateNeurophoto = runtimeActions.some((a: any) => a.name === "GENERATE_NEUROPHOTO");
      const actions = hasGenerateNeurophoto ? runtimeActions : [...runtimeActions, generateImageAction];

      // DEBUG: Логируем доступные actions
      console.log(`🔍 [Actions] Всего actions: ${actions.length}, hasGenerateNeurophoto: ${hasGenerateNeurophoto}, names: ${actions.map((a: any) => a.name).join(', ')}`);
      ...
      const adminActions = [
        ...
        "GENERATE_NEUROPHOTO", // LoRA генерация из plugin-vibe-face-avatar
      ];
      ...
      for (const action of actions) {
        ...
        try {
          ...
          if (isValid) {
            KolsLogger.success(
              `🎯 Admin Action "${action.name}" прошёл validate для "${text}"`
            );
  ...
  private async generateAndSendReply(
    ...
    try {
      KolsLogger.activity(
        `Генерация ответа для ${processedMessage.fromFirstName}...`
      );

      // 🔍 RAG: Ищем релевантный контент из Knowledge Base
      const messageText = processedMessage.text || "";
      const relevantChunks = this.knowledgeProvider.searchContent(
        messageText,
        3
      );
      ...
      if (relevantChunks.length > 0) {
        ragContext =
          ...
          relevantChunks
            ...
            .join("\n---\n");
        KolsLogger.success(
          `RAG: найдено ${relevantChunks.length} релевантных чанков`
        );
      }
      ...
      try {
        ...
        conversationHistory = memories
          ...
          .map((m) => ({
            role: (m.entityId === this.runtime!.agentId
              ? "assistant"
        ...
        if (conversationHistory.length > 0) {
          KolsLogger.info(
            `📜 История: загружено ${conversationHistory.length} сообщений`
          );
        }
      } catch (historyError) {
      ...
      if (chatConfig) {
        ...
        const promptBuilder =
          this.runtime?.getService<PromptBuilderService>("prompt-builder");

        if (promptBuilder) {
          const context = promptBuilder.createMessageContext(
            senderName,
            senderUsername.replace("@", ""), // убираем @ для контекста
            processedMessage.tgChatTitle
          );
          systemPrompt = promptBuilder.buildSystemPrompt(chatConfig, context);
          KolsLogger.success(
            `🎯 Использую динамический промпт из ChatConfig (persona: ${chatConfig.personaName})`
          );
        } else {
          // Fallback на промпт из конфига напрямую
          systemPrompt =
            chatConfig.systemPrompt ||
            this.getDefaultSystemPrompt(senderInfo, historyContext, ragContext);
          KolsLogger.info(
            `📝 Использую systemPrompt из ChatConfig (без PromptBuilder)`
          );
        }
      ...
      try {
        ...
        llmReply = await this.runtime.useModel(ModelType.TEXT_SMALL, {
          prompt: fullPrompt,
          temperature: 0.7,
          maxTokens: 200, // 🔒 Уменьшено с 500 до 200 (доступно только 230 токенов)
        });

        KolsLogger.success(
          `✅ ElizaOS useModel вернул ответ: ${llmReply?.substring(0, 50)}...`
        );
      } catch (modelError) {
        ...
        const openrouterKey = envFileKey || envKey;
        ...
        const response = await fetch(
          "https://openrouter.ai/api/v1/chat/completions",
          {
            ...
            body: JSON.stringify({
              model: "x-ai/grok-4.1-fast", // 🚀 Быстрая платная модель
              messages: [
        ...
        if (!response.ok) {
          ...
          KolsLogger.error(
            `OpenRouter ошибка: ${response.status} - ${errorText}`
          );
          return;
        }
      ...
      KolsLogger.success(
        `🎯 LLM ответ получен: "${cleanedReply.substring(0, 80)}..."`
      );
      ...
      KolsLogger.info(
        `📤 Отправка ${messageParts.length} сообщений в chatId: ${processedMessage.chatId}`
      );
      ...
      for (let i = 0; i < messageParts.length; i++) {
        ...
        lastResult = result;
        console.log(
          `📤 [TelegramService] Part ${i + 1}/${messageParts.length} sent:`,
      ...
      if (lastResult.success) {
        try {
          const agentMemory: Memory = {
            id: stringToUuid(
              `telegram-agent-${processedMessage.chatId}-${lastResult.messageId || Date.now()}`
            ),
        ...
        // 📩 ПЕРЕСЫЛКА ДИАЛОГА: ВРЕМЕННО для теста - пересылаем ВСЕ ответы
        // Игнорируем категории триггеров
        
        // 🛠️ HARDCODED ADMIN CHAT ID for testing (Use AI STARS or other group)
        // Если конфига нет - шлем в этот чат.
        // ID из логов (AI STARS админ? нет инфо, берем рандомный или просим юзера)
        // While testing, let's look at logs content more closely later.
        // For now, assume user has config or we use a placeholer if missing.
        const targetForwardChatId = chatConfig?.forwardChatId || "2298297094"; // Fallback to NeuroMentor channel itself as loopback test if missing

        if (targetForwardChatId && processedMessage.text) {
          try {
            // Для теста находим любые триггеры просто для инфо
            const triggers = findTriggers(processedMessage.text);
            const categories = triggers.map(t => getTriggerCategory(t) || "UNKNOWN");
            
            // Если триггеров нет - добавляем пометку что это авто-ответ
            if (triggers.length === 0) {
              triggers.push("AUTO_REPLY");
              categories.push("ALL");
            }

            KolsLogger.info(`📩 [DialogForward] ПЕРЕСЫЛКА ВСЕХ ОТВЕТОВ (Тестовый режим) в ${targetForwardChatId}`);

            const metadata: DialogMetadata = {
              sourceChatId: processedMessage.chatId,
              sourceTgChatTitle: processedMessage.tgChatTitle || `Chat ${processedMessage.chatId}`,
              userName: processedMessage.fromFirstName || "Unknown",
              userUsername: processedMessage.fromUsername,
              userText: processedMessage.text,
              agentText: cleanedReply,
              triggers,
              triggerCategories: categories,
              timestamp: new Date(),
            };

            console.log(`📩 [DialogForward] ПЕРЕСЫЛКА ОТВЕТОВ в ${targetForwardChatId}`);

            await this.dialogForwardService.forwardDialog(
              processedMessage.messageId, // ID сообщения юзера
              lastResult.success && lastResult.messageId
                ? lastResult.messageId
                : 0, // ID ответа агента (если есть)
              targetForwardChatId,
              metadata,
              this.adapter // 🔥 FIX: Явно передаем адаптер!
            );
            console.log(`📩 [DialogForward] Диалог переслан в ${targetForwardChatId}`);
          } catch (forwardError) {
             KolsLogger.warn(`📩 [DialogForward] Ошибка пересылки: ${forwardError}`);
          }
        }
      }
      ...
      const fallbackReply = this.generateVibeeReply(
        processedMessage.text,
        processedMessage.fromFirstName,
        ragContext
      );
  ...
  private generateVibeeReply(
    ...
    if (
      text.includes("вайбкодинг") ||
      text.includes("vibecoding") ||
...
Что дальше хочешь сделать? Я готов помочь с новыми вызовами!`;
    ...
    // Сложно
    if (
      text.includes("сложно") ||
      text.includes("не понимаю") ||
      text.includes("трудно")
    ) {
...
Ты справишься, я в тебя верю!`;
    ...
    if (
      text.includes("агент") ||
      text.includes("elizaos") ||
      text.includes("элизаос") ||
      text.includes("бот")
    ) {
...
Расскажи подробнее что именно хочешь узнать? Чем конкретнее вопрос - тем точнее ответ!`;
  ...
  private getDefaultSystemPrompt(
    ...
    return `Ты VIBEE - AI-эксперт по разработке ботов, автоматизации и ElizaOS.

КТО ПИШЕТ: ${senderInfo}

СТИЛЬ ОТВЕТОВ:
- Отвечай по существу, без воды
- Используй структуру когда это помогает (списки, пункты)
- Будь дружелюбным, но профессиональным
- Давай конкретные решения, а не отмазки
- Если не знаешь точно - честно скажи и предложи альтернативу

ФОРМАТ:
- Краткие абзацы (2-3 предложения)
- Списки для перечислений
- Код в блоках \`\`\` когда нужно
- Эмодзи умеренно, к месту
- Отвечай на русском

ЗАПРЕЩЕНО:
- Пустые фразы: "это интересный вопрос", "давай разберёмся"
- Отмазки вместо ответа
- Слишком длинные ответы (max 300 слов)
...
${historyContext ? `КОНТЕКСТ ЧАТА:${historyContext}` : ""}

${ragContext ? `БАЗА ЗНАНИЙ:\n${ragContext}` : ""}`;
  }
  ...
  onMessage(handler: (message: any) => void): void {
    ...
    console.log(
      `📨 [TelegramService] Подписка на сообщения установлена (${this.messageHandlers.size} обработчиков)`
    );

    // 🔒 ЗАЩИТА ОТ ДУБЛИРОВАНИЯ: Регистрируем обработчик только один раз
    if (!this.messageHandlerRegistered && this.adapter?.onMessage) {
      console.log(
        "✅ [TelegramService] Adapter supports onMessage, registering handler..."
      );
      // Связываем обработчик адаптера с нашим сервисом
      const adapterHandler = (event: any) => {
        console.log(
          "📥 [TelegramService] Adapter handler called, forwarding to handleIncomingMessage"
        );
      ...
      console.log(
        "✅ [TelegramService] Handler registered with adapter (ONCE ONLY)"
      );
    } else if (!this.adapter?.onMessage) {
      console.log("⚠️ [TelegramService] Adapter does NOT support onMessage!");
    } else {
      console.log(
        `🔒 [TelegramService] Handler already registered (skipping duplicate registration)`
      );
    }
  }
  ...
  offMessage(handler: (message: any) => void): void {
    ...
    console.log(
      `📨 Отписка от сообщений (${this.messageHandlers.size} обработчиков)`
    );
  }
    ...
    try {
      ...
      return dialogs
        ...
        .map((dialog) => ({
          ...
          isMonitored:
            this.monitoredGroups.has(dialog.id) ||
            this.monitoredGroups.has(dialog.name),
        }));
    } catch (error) {
   ...
   */
  private recentMessages: IRecentMessage[] = [];

  // Assuming repetitionMap is a private property of the class
  private repetitionMap: Map<string, { count: number; lastText: string; lastTime: number }>;

  /**
   * Обрабатывает входящее сообщение
   */
  private async handleIncomingMessage(event: any): Promise<void> {
    // 🛠️ FIX: MTProtoAdapter returns a normalized object, not a raw GramJS event with .message prop
    // The 'event' object IS the message payload
    const text = event.text || "";
    
    // 🛡️ RECURSION GUARD: Ignore own forwarded logs
    if (text.startsWith("📩 **Новый лид")) {
       console.log("🛡️ [Anti-Recursion] Ignoring system log message.");
       return;
    }

    const senderId = event.fromId?.toString();
    const chatId = event.chatId?.toString();
    
    if (!senderId || !chatId) return;

    // 🛡️ REPETITION GUARD
    if (!this.repetitionMap) {
        this.repetitionMap = new Map();
    }
    
    // Key = chatId:userId
    const repKey = `${chatId}:${senderId}`;
    const normalizedText = text.trim().toLowerCase();
    
    const repState = this.repetitionMap.get(repKey);
    let loopDetected = false;
    let loopCount = 0;

    if (repState && repState.lastText === normalizedText) {
        // Repeated message
        loopCount = repState.count + 1;
        this.repetitionMap.set(repKey, { ...repState, count: loopCount, lastTime: Date.now() });
        if (loopCount > 2) {
            loopDetected = true;
            console.warn(`🔄 [Anti-Loop] User ${senderId} repeated "${normalizedText}" ${loopCount} times.`);
        }
    } else {
        // New message - reset
        this.repetitionMap.set(repKey, { count: 1, lastText: normalizedText, lastTime: Date.now() });
    }

    // 🛡️ ANTI-LOOP PROTECTION: Block repeated messages WITHOUT triggers
    // Allow repeated messages if they contain crypto triggers
    if (loopDetected && !containsTrigger(text)) {
        console.log(`🔄 [Anti-Loop] Blocking repeated message without triggers: "${normalizedText}"`);
        return;
    }

    console.log(`✅ [Anti-Loop] Message passed anti-loop check, proceeding with processing`);

    // Pass context to generation logic
              const tgChatTitle = `Chat ${chatId}`; // Simplified since we don't have .getChat() on the event    const username = senderId; // Simplified
    ...
    console.log(`📝 [TelegramService] Message saved to history, preparing for processing`);
    
    // Construct a pseudo-message object for compatibility with existing logic if needed
    // or just pass extracted data.
    // However, looking at generateMessage signature in runtime, it likely takes text.
    // The rest of this method (which was hidden in previous view) likely needs 'message' object.
    // Since we don't have the original message object, we might need to adapt downstream logic.
    // But since I can't see the downstream logic fully, I will define 'message' as the event itself for now
    // and hope downstream uses .text, .chatId etc which match.
    ...
    // Now process the message using the main logic
    const processedMessage = message;
    const finalChatId = chatId;
              const tgChatTitle = `Chat ${chatId}`; // Simplified    const fromUserId = senderId;
    const fromFirstName = "User"; // Simplified
    const usernameDisplay = ""; // Simplified

    console.log(`🔄 [TelegramService] Starting main message processing for chat ${finalChatId}`);

    // Get chat config
    let chatConfig: ChatConfig | null = null;
    try {
      chatConfig = await this.chatConfigService.getChatConfig(finalChatId);
    } catch (configError) {
      console.log(`⚠️ [TelegramService] Could not get chat config: ${configError}`);
    }

    // Main processing logic from the beginning of the file
    try {
      const staticConfig = AGENTS_CONFIG.vibee?.targetChats.find(c => c.chatId === finalChatId);
      if (staticConfig?.forwardChatId && chatConfig?.forwardChatId !== staticConfig.forwardChatId) {
           console.log(`🔄 [TelegramService] Force updating forwardChatId from static config: ${chatConfig?.forwardChatId} -> ${staticConfig.forwardChatId}`);
   ...
   * Получить последние сообщения для админки
   */
  getRecentMessages(limit: number = 50): IRecentMessage[] {
  ...
  initializeAuthService(): TelegramAuthService | null {
    ...
    if (!apiId || !apiHash) {
      console.warn(
        "[TelegramService] Cannot initialize AuthService - missing TELEGRAM_API_ID/HASH"
      );
      return null;
    }
  ...
  async getOrCreateUserAdapter(userId: string): Promise<MTProtoAdapter | null> {
    ...
    if (!authService) {
      console.warn(
        `[TelegramService] AuthService not available for user ${userId}`
      );
      return null;
    }
    ...
    try {
      ...
      if (!connected) {
        console.error(
          `[TelegramService] Failed to connect user adapter for ${userId}`
        );
        return null;
      }
      ...
      console.log(
        `[TelegramService] User adapter created and connected for ${userId}`
      );
  ...
  async sendMessageAsUser(
    ...
    if (!adapter) {
      return {
        success: false,
        error:
          "User adapter not available. User may need to connect their Telegram account.",
      };
    }
    ...
    try {
      ...
      console.error(
        `[TelegramService] Error sending message as user ${userId}:`,
        error
      );
      return { success: false, error: String(error) };
    }
  ...
  async getUserDialogs(
    ...
    try {
      ...
      console.error(
        `[TelegramService] Error getting dialogs for user ${userId}:`,
        error
      );
      return [];
    }
  ...
  async getUserTelegramInfo(userId: string): Promise<ITelegramUser | null> {
    ...
    try {
      ...
      console.error(
        `[TelegramService] Error getting user info for ${userId}:`,
        error
      );
      return null;
    }
  ...
  async disconnectUserAdapter(userId: string): Promise<void> {
    ...
    if (adapter) {
      try {
        ...
        console.error(
          `[TelegramService] Error disconnecting user adapter:`,
          error
        );
      }
  ...
  private async handlePhotoSession(
    ...
    try {
      ...
      const photoService =
        this.runtime.getService<PhotoSessionService>("photo-session");
      if (!photoService) {
        // Сервис не зарегистрирован - пропускаем
        console.log(
          "⚠️ [PhotoSession] PhotoSessionService не найден в runtime!"
        );
        return false;
      }
      ...
      console.log(
        `🔍 [PhotoSession] Проверка: hasMedia=${hasMedia}, hasText=${hasText}, hasPhotosInSession=${photoService.hasPhotos(chatId, userId)}`
      );
      ...
      if (hasMedia) {
        ...
        if (photos.length > 0) {
          const count = photoService.addPhotos(
            chatId,
            userId,
            photos,
            message.id
          ...
          const confirmMsg = await this.generatePhotoConfirmation(
            count,
            photos.length
          );
          await this.sendMessage(chatId, confirmMsg);
          console.log(
            `📸 [PhotoSession] ${chatId}/${userId}: добавлено ${photos.length} фото, всего ${count}`
          );
          return true;
        }
      }
      ...
      if (hasReplyMedia && hasText && message.replyToMsgId) {
        ...
        if (replyPhotos.length > 0) {
          ...
          console.log(
            `📸 [PhotoSession] Reply mode: ${replyPhotos.length} фото из reply, промпт: "${message.text?.substring(0, 50)}..."`
          );

          // Запускаем генерацию через NanoBanana
          await this.triggerPhotoGeneration(
            chatId,
            userId,
            replyPhotos,
            message.text
      ...
      // Триггеры для генерации (общие для режимов 3 и 4)
      // Русские и английские команды для запуска генерации
      ...
      const hasGenerationTrigger = generationTriggers.some((t) =>
        lowerText.includes(t)
      );

      // Режим 3: Текст С ТРИГГЕРОМ + накопленные фото от самого пользователя
      if (
        hasText &&
        hasGenerationTrigger &&
        photoService.hasPhotos(chatId, userId)
      ) {
        const sessionPhotos = photoService.getPhotos(chatId, userId);
        const prompt = message.text;

        console.log(
          `📸 [PhotoSession] Session mode (user): ${sessionPhotos.length} фото в сессии, промпт: "${prompt?.substring(0, 50)}..."`
        );
      ...
      if (
        hasText &&
        hasGenerationTrigger &&
        photoService.hasAnyPhotosInChat(chatId)
      ) {
        const { photos: allPhotos, userIds } =
          photoService.getAllPhotosFromChat(chatId);
        const prompt = message.text;

        console.log(
          `📸 [PhotoSession] Group mode: ${allPhotos.length} фото от ${userIds.length} пользователей, промпт: "${prompt?.substring(0, 50)}..."`
        );
      ...
      // Режим 5: "Ещё варианты" - повторная генерация с последними использованными фото
      // Триггеры для повторной генерации (русские и английские)
      ...
      const hasRepeatTrigger = repeatTriggers.some((t) =>
        lowerText.includes(t)
      );
      ...
      if (
        hasText &&
        hasRepeatTrigger &&
        photoService.hasLastUsedPhotos(chatId, userId)
      ) {
        const lastPhotos = photoService.getLastUsedPhotos(chatId, userId);
        // Используем сохранённый промпт вместо текста "ещё варианты"
        const savedPrompt = photoService.getLastUsedPrompt(chatId, userId);
        const prompt = savedPrompt || message.text;

        console.log(
          `📸 [PhotoSession] Repeat mode: ${lastPhotos.length} последних фото, промпт: "${prompt?.substring(0, 50)}..." (saved: ${!!savedPrompt})`
        );

        // Запускаем повторную генерацию
        await this.triggerPhotoGeneration(chatId, userId, lastPhotos, prompt);
        return true;
      }
  ...
  private async triggerPhotoGeneration(
    ...
    try {
      ...
      if (!nanoBanana || !nanoBanana.isAvailable()) {
        ...
        await this.sendMessage(
          chatId,
          "Сервис генерации временно недоступен. Проверьте REPLICATE_API_KEY."
        );
        return;
      }

      console.log(
        `🍌 [PhotoSession] Generating with ${photos.length} photos, aspect: ${aspectRatio}, resolution: ${resolution}, prompt: "${prompt.substring(0, 50)}..."`
      );

      // Генерируем через NanoBananaService напрямую
      const result =
        photos.length >= 2
          ? await nanoBanana.blendImages(photos, prompt, {
              aspectRatio,
              resolution,
            })
      ...
      const photoService =
        this.runtime.getService<PhotoSessionService>("photo-session");
      if (photoService) {
        photoService.saveLastUsedPrompt(chatId, userId, prompt);
      }

      // Отправляем результат с подсказкой про повторную генерацию
      const caption =
        photos.length > 1
          ? `Создано из ${photos.length} фото: ${prompt}\n\nНапиши "ещё" для нового варианта`
   ...
   */
  private async generatePhotoConfirmation(
    totalCount: number,
    addedCount: number
  ): Promise<string> {
    if (!this.runtime) {
...
Напиши ТОЛЬКО подтверждение:`;
  ...
  private async checkAndHandleCryptoPayment(processedMessage: {
    ...
    try {
      ...
      const cryptoService =
        this.runtime.getService<CryptoPaymentService>("crypto-payment");
      if (!cryptoService) {
      ...
      KolsLogger.info(
        `💎 Обнаружен крипто-платёж: ${payment.amount} ${payment.currency}`
      );
      ...
      if (credits <= 0) {
        KolsLogger.warn(
          `Платёж ${payment.amount} ${payment.currency} дал 0 кредитов`
        );
        return true; // Обработано, но без зачисления
      }

      // Получаем PaymentService для зачисления
      const paymentServiceForCrypto =
        this.runtime.getService<PaymentService>("payment");
      if (!paymentServiceForCrypto) {
        KolsLogger.error(
          "PaymentService не найден - невозможно зачислить кредиты"
        );
        return true;
      }
      ...
      paymentServiceForCrypto.addPhotoCredits(
        recipientUserId,
        credits,
        "crypto",
        {
      ...
      const successMessage = cryptoService.generateSuccessMessage(
        payment,
        credits
      );
      ...
      KolsLogger.success(
        `✅ Зачислено ${credits} фото-кредитов пользователю ${recipientUserId}`
      );
