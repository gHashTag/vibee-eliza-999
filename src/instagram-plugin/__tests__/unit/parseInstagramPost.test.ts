...
describe('parseInstagramPost - Парсинг сообщений Instagram', () => {
  ...
  it('должно бросать ошибку если нет изображения', async () => {
    ...
    await expect(parseInstagramPost(text)).rejects.toThrow('Прикрепите изображение как файл или укажите URL изображения в тексте сообщения');
  });
  ...
  it('должно очищать подпись от лишних пробелов', async () => {
    ...
    const result = await parseInstagramPost(text, [createImageAttachment('https://example.com/img.jpg')]);
