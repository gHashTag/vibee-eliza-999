...
describe("parseInstagramPost - Парсинг сообщений Instagram", () => {
  it("должно извлекать URL изображения из текста", async () => {
    const text =
      "Опубликуй пост в Instagram с изображением https://example.com/image.jpg и подписью красивый закат";
    const result = await parseInstagramPost(text);

    expect(result.imageUrl).toBe("https://example.com/image.jpg");
    expect(result.caption).toBe("красивый закат");
  });

  it("должно извлекать URL из вложения", async () => {
    const text = "Опубликуй это в Instagram";
    const attachments = [
      createImageAttachment("https://example.com/photo.jpg"),
    ];
    const result = await parseInstagramPost(text, attachments);
  ...
  it("должно предпочитать URL из текста URL из вложения", async () => {
    const text = "Опубликуй пост https://text-url.com/image.jpg";
    const attachments = [
      createImageAttachment("https://attachment-url.com/photo.jpg"),
    ];
    const result = await parseInstagramPost(text, attachments);
  ...
  it("должно извлекать хэштеги из текста", async () => {
    const text =
      "Опубликуй пост с #тег и #другим #хэштегом https://example.com/image.jpg";
    const result = await parseInstagramPost(text);
  ...
  it("должно использовать default caption если он пустой", async () => {
  ...
  it('должно правильно парсить команду "опубликуй"', async () => {
    const text =
      "Опубликуй пост в Instagram с изображением https://example.com/image.jpg и подписью тестовая подпись";
    const result = await parseInstagramPost(text);
  ...
  it('должно работать с командой "/instagram"', async () => {
    const text =
      "/instagram пост с картинкой https://example.com/image.jpg и текстом привет мир";
    const result = await parseInstagramPost(text);
  ...
  it("должно обрабатывать несколько изображений в attachments (берет первое)", async () => {
    const text = "Опубликуй это в Instagram";
    const attachments = [
  ...
  it("должно бросать ошибку если нет изображения", async () => {
    const text = "Опубликуй пост в Instagram";

    await expect(parseInstagramPost(text)).rejects.toThrow(
      "Прикрепите изображение как файл или укажите URL изображения в тексте сообщения",
    );
  });

  it("должно работать с различными форматами изображений", async () => {
    const testCases = [
  ...
  it('должно корректно работать с attachment type "photo"', async () => {
    const text = "Опубликуй это в Instagram";
    const attachments = [
      {
        type: "photo",
  ...
  it("должно обрабатывать длинные подписи", async () => {
    const longCaption =
      "Это очень длинная подпись для поста в Instagram".repeat(10);
  ...
  it("должно очищать подпись от лишних пробелов", async () => {
    const text = "  Опубликуй    пост   с подписью   тест   ";
    const result = await parseInstagramPost(text, [
      createImageAttachment("https://example.com/img.jpg"),
    ]);
