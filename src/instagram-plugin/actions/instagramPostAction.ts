...
export const instagramPostAction: Action = {
  ...
  handler: async (runtime, message, state, options, callback) => {
    try {
      ...
      console.log(
        "🐝 [Instagram] Сообщение от:",
        message.content?.userId || message.content?.source || "Unknown",
      );
      console.log(
        "🐝 [Instagram] Текст сообщения:",
        message.content?.text?.substring(0, 100),
      );
      console.log(
        "🐝 [Instagram] Вложения:",
        JSON.stringify(message.content?.attachments || [], null, 2),
      );
      ...
      if (!storageService) {
        console.warn(
          "⚠️ [Instagram] StorageService не найден, используем прямые URL",
        );
      }
...
export async function parseInstagramPost(
  ...
  if (!imageUrl && attachments && attachments.length > 0) {
    ...
    if (imageAttachment?.url) {
      console.log(
        "📝 [Instagram] Найдено изображение во вложениях:",
        imageAttachment.url,
      );
      ...
      if (storageService) {
        ...
        try {
          ...
          console.log(
            "✅ [Instagram] Файл загружен в облако:",
            uploadResult.url,
          );
        } catch (error) {
          console.warn(
            "⚠️ [Instagram] Не удалось загрузить в облако, используем оригинальный URL:",
            error,
          );
  ...
  if (captionMatch) {
    ...
    cleanText = cleanText.replace(
      /\b(?:с\s+(?:подписью|изображением|текстом|картинкой|фотографией|фото)\s*и\s*)?(?:подпись|подписи|подписью)\s*[:\-]?\s*/gi,
      "",
    );
  ...
  const hashtags = hashtagMatches
    ? hashtagMatches.map((tag) => tag.substring(1).toLowerCase())
    : [];
