...
fn check_telegram_auth() -> Result(Nil, ToolResult) {
  let url = bridge_url <> "/api/v1/auth/status"
  case http_get(url) {
    Error(_) -> {
    ...
    Ok(body) -> {
      ...
      case json.parse(from: body, using: session_count_decoder()) {
        Ok(count) if count > 0 -> Ok(Nil)
        _ -> Error(auth_required_result("Telegram не авторизован."))
      }
    }
  }
}
...
fn http_get(url: String) -> Result(String, String) {
  let req =
    ...
    |> request.set_path(string.replace(url, "http://localhost:8081", ""))
    ...
    Ok(parsed) -> {
      case validation.validate_phone(parsed.phone) {
        ...
        Ok(phone) -> {
          ...
          case make_telegram_post_request(url, body) {
            Ok(response) -> {
              logging.info("[AUTH] Verification successful")
              protocol.text_result(response)
            }
            Error(err) -> {
...
fn make_telegram_post_request(
  ...
  let req =
    ...
    |> request.set_host("localhost")
    |> request.set_port(8081)
    |> request.set_path(string.replace(url, bridge_url, ""))
    |> request.set_header("content-type", "application/json")
    |> request.set_body(body)
